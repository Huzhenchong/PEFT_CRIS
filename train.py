    ...
    for name, param in model.named_parameters():
        ...
        if not param.requires_grad:
            ...
            if torch.distributed.get_rank() == 0:
                print(f"Frozen: {name}")
    if torch.distributed.get_rank() == 0:
        print(f"\nFrozen params: {frozen}/{total} ({100.*frozen/total:.2f}%)")
...
def main_worker(gpu, args):
    ...
    # 冻结策略调整
    for name, param in model.named_parameters():
        if "backbone" in name and "visual_adapters" not in name and "learnable_prompt" not in name:
            param.requires_grad = False  # 冻结原始CLIP
        else:
            param.requires_grad = True  # 训练Adapter和Prompt
